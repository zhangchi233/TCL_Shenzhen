...
TAXONOMY_CSV = """
一级分类,二级分类,三级细项
器件 (Devices),类型 (Types),Micro-LED display
器件 (Devices),类型 (Types),微型LED显示 (MicroLED)
器件 (Devices),类型 (Types),Mini LED
器件 (Devices),类型 (Types),OLED (有机发光二极管)
器件 (Devices),类型 (Types),WOLED
器件 (Devices),类型 (Types),Micro OLED
器件 (Devices),类型 (Types),OLED-on-Silicon
器件 (Devices),类型 (Types),硅基 OLED
器件 (Devices),类型 (Types),TFT-LCD
器件 (Devices),类型 (Types),GaN LED / InGaN LED / AlGaInP LED / GaAs LED
器件 (Devices),形态 (Forms),柔性显示 (Flexible Display)
器件 (Devices),形态 (Forms),柔性技术
器件 (Devices),形态 (Forms),透明显示
器件 (Devices),形态 (Forms),全息显示
器件 (Devices),形态 (Forms),光场显示
器件 (Devices),形态 (Forms),立体显示 (3D显示)
器件 (Devices),形态 (Forms),空间显示
器件 (Devices),形态 (Forms),平板显示
器件 (Devices),形态 (Forms),微显示器 (Micro-display)
核心 (Core),背板 (Backplane),薄膜晶体管 (TFT)
核心 (Core),背板 (Backplane),非晶硅TFT (a-Si TFT)
核心 (Core),背板 (Backplane),低温多晶硅 (LTPS TFT)
核心 (Core),背板 (Backplane),氧化物TFT (IGZO TFT / Metal Oxide TFT)
核心 (Core),背板 (Backplane),有源驱动显示 (AM-Driving)
核心 (Core),电路 (Circuits),像素电路 (Pixel Circuit)
核心 (Core),电路 (Circuits),像素驱动电路
核心 (Core),电路 (Circuits),GOA电路 (Gate Driver on Array)
核心 (Core),电路 (Circuits),像素电极
核心 (Core),电路 (Circuits),Level Shifter
核心 (Core),效应 (Effects),短沟道效应 (Short Channel Effect)
核心 (Core),效应 (Effects),热载流子效应 (Hot Carrier Effect)
核心 (Core),效应 (Effects),Kink Effect (扭曲效应)
材料 (Materials),液晶 (LC),液晶材料
材料 (Materials),液晶 (LC),蓝相液晶 (Blue Phase LC)
材料 (Materials),液晶 (LC),正性液晶 / 负性液晶
材料 (Materials),液晶 (LC),液晶取向
材料 (Materials),有机 (Organic),OLED材料科学
材料 (Materials),有机 (Organic),OLED发光材料 (Emitters)
材料 (Materials),有机 (Organic),磷光 OLED (Phosphorescent)
材料 (Materials),有机 (Organic),TADF (热活化延迟荧光)
材料 (Materials),量子 (Quantum),QLED (量子点显示)
材料 (Materials),量子 (Quantum),QLED材料科学
材料 (Materials),量子 (Quantum),量子点膜 (QDEF)
材料 (Materials),量子 (Quantum),量子点背光源
材料 (Materials),光膜 (Films),偏光片 (Polarizer)
材料 (Materials),光膜 (Films),偏光片技术
材料 (Materials),光膜 (Films),彩色滤光片 (Color Filters)
材料 (Materials),基材 (Base),柔性聚酰亚胺 (Flexible PI)
材料 (Materials),基材 (Base),Chip On PI
工艺 (Process),成膜 (Deposition),化学气相沉积 (CVD)
工艺 (Process),成膜 (Deposition),物理气相沉积 (PVD)
工艺 (Process),成膜 (Deposition),蒸镀技术 (Evaporation)
工艺 (Process),成膜 (Deposition),喷墨打印技术 (Inkjet Printing)
工艺 (Process),光刻 (Patterning),光刻技术 (Photolithography)
工艺 (Process),光刻 (Patterning),干法刻蚀 (Dry Etching)
工艺 (Process),光刻 (Patterning),湿法刻蚀 (Wet Etching)
工艺 (Process),光刻 (Patterning),制程工艺
工艺 (Process),热程 (Thermal),退火 (Annealing)
工艺 (Process),热程 (Thermal),准分子激光退火 (ELA)
工艺 (Process),热程 (Thermal),快速热退火 (RTA)
工艺 (Process),取向 (Align),摩擦配向 (Rubbing)
工艺 (Process),取向 (Align),光配向 (Photo-alignment)
工艺 (Process),取向 (Align),配向膜 (Alignment Layer)
工艺 (Process),封装 (Encap),薄膜封装 (TFE)
工艺 (Process),封装 (Encap),封装技术
工艺 (Process),封装 (Encap),OLED 封装
工艺 (Process),集成 (Assembly),芯片转移 (Mass Transfer)
工艺 (Process),显示,芯片控制
工艺 (Process),集成 (Assembly),集成技术
器件 (Devices),液晶技术,液晶显示 LCD
模组 (Module),背光 (Backlight),背光模组
模组 (Module),背光 (Backlight),直下式背光 / 侧入式背光
模组 (Module),背光 (Backlight),Local Dimming (局部调光)
模组 (Module),背光 (Backlight),动态调光
模组 (Module),背光 (Backlight),亮度均匀性
模组 (Module),驱动 (Driver),Source Driver (源极驱动)
模组 (Module),驱动 (Driver),TCON (时序控制器)
模组 (Module),驱动 (Driver),Chip On Film (COF)
模组 (Module),驱动 (Driver),Chip On Glass (COG)
模组 (Module),触控 (Touch),触控技术
模组 (Module),触控 (Touch),On-cell 触控
模组 (Module),触控 (Touch),电容触控
模组 (Module),接口 (Interface),LVDS / mini-LVDS
模组 (Module),接口 (Interface),V-by-One
评估 (Evaluation),参数 (Specs),色域 (Color Gamut)
评估 (Evaluation),参数 (Specs),可视角 (Viewing Angle)
评估 (Evaluation),参数 (Specs),光学规格
评估 (Evaluation),缺陷 (Defects),亮点缺陷
评估 (Evaluation),缺陷 (Defects),画面串扰 (Crosstalk)
评估 (Evaluation),缺陷 (Defects),像素补偿 / 灰阶补偿
不确定, 不确定,其他类型
...
def parse_taxonomy(csv_text="/mnt/storage/dataset/PPVL_reuslts_CN/RAG-Anything/taxonomy/test.csv"):
    ...
    tags = []
    titles = "一级分类,二级分类,三级细项"
    for _, row in df.iterrows():
        tags.append(f"{row['一级分类']} > {row['二级分类']} > {row['三级细项']}")
    return tags
...
def build_system_prompt(taxonomy_list):
    ...
    请从提供的知识库中，精准识别出该数据对应的 **三级分类标签 (L3 Category Tag)** 和 **图片类型 (Image Type)**。
    ...
    - 结合【图片内容】和【文本描述】锁定具体的 **三级标签 (L3 Tag)**。
    - **同义词推断**: 如果文本提到 "Foldable Phone" (折叠手机)，你应该选择 "柔性显示 (Flexible Display)"。
    - **精确性**: 必须精确到 L3 细项。如果图片明显是 "Micro-LED"，不要只选宽泛的 "器件 (Devices)"。
    ...
    "l3_tag": "精准匹配到的三级细项名称 (例如 'Micro-LED display')",
    "l1_category": "对应的一级分类名称 (例如 '器件 (Devices)')"
    }}
    """
...
async def main():
    ...
    #TABLE_NAME = "merged_results_diagrams_paths_29_12_407"
    TABLE_NAME = "semiconductor_batch_4_1"
    print("🔄 Connecting to DB and fetching data...")
    # 选取包含两种结果的数据，以便进行对比
    # 注意：这里假设数据库里已经是 JSON 字符串或者 STRUCT，fetchdf 会将其转为 python object 或 string
    query = f"""
    ...
    # 采样 50 条用于测试
    if len(data) > 50:
        dataset = data.sample(n=50).to_dict('records')
    else:
        dataset = data.to_dict('records')
    
    print(f"🚀 准备对 {len(dataset)} 条数据进行双向评估 (With/Without Metadata)...")

    # 3. 初始化 Pipeline
    pipeline = AsyncLabelingPipeline(api_key=API_KEY, taxonomy_csv=TAXONOMY_CSV)
    
    # 定义一个辅助函数来处理单行数据的双重评估
    async def evaluate_single_row(row):
        tasks = []
        
        # --- A. 评估 With Metadata Results ---
        res_with = row.get('with_meta_data_results')
        # 如果是 JSON 字符串则解析，如果是 dict 则直接用
        if isinstance(res_with, str):
            try:
                res_with = json.loads(res_with)
            except:
                res_with = {}
        
        if res_with and isinstance(res_with, dict):
            q_with = res_with.get('generated_question')
            a_with = res_with.get('generated_answer')
            # 添加任务：评估 With Meta
            tasks.append(pipeline.filterout_data(row, q_with, a_with))
        else:
            # 占位符，保持索引对齐
            tasks.append(asyncio.sleep(0, result={"pass": False, "score": 0.0, "reason": "No Data"}))

        # --- B. 评估 Without Metadata Results ---
        res_without = row.get('without_meta_data_results')
        if isinstance(res_without, str):
            try:
                res_without = json.loads(res_without)
            except:
                res_without = {}
        
        if res_without and isinstance(res_without, dict):
            q_without = res_without.get('generated_question')
            a_without = res_without.get('generated_answer')
            # 添加任务：评估 Without Meta
            tasks.append(pipeline.filterout_data(row, q_without, a_without))
        else:
             tasks.append(asyncio.sleep(0, result={"pass": False, "score": 0.0, "reason": "No Data"}))
        ...
        # 结果解包
        res_with_eval = eval_results[0] if len(eval_results) > 0 else {}
        res_without_eval = eval_results[1] if len(eval_results) > 1 else {}
        
        # 将评估结果写回 row (扁平化以便存 CSV)
        row['filter_with_meta_pass'] = res_with_eval.get('pass')
        row['filter_with_meta_score'] = res_with_eval.get('score')
        row['filter_with_meta_reason'] = res_with_eval.get('reason')
        
        row['filter_without_meta_pass'] = res_without_eval.get('pass')
        row['filter_without_meta_score'] = res_without_eval.get('score')
        row['filter_without_meta_reason'] = res_without_eval.get('reason')
        
        # 计算分数差 (With - Without)
        if isinstance(row['filter_with_meta_score'], float) and isinstance(row['filter_without_meta_score'], float):
            row['score_improvement'] = row['filter_with_meta_score'] - row['filter_without_meta_score']
        else:
            row['score_improvement'] = 0.0

        return row

    # 4. 批量运行评估
    # 创建所有行的任务
    all_tasks = [evaluate_single_row(row) for row in dataset]
    
    # 并发执行
    processed_dataset = await asyncio.gather(*all_tasks)
    
    # 5. 保存结果到 CSV
    output_df = pd.DataFrame(processed_dataset)
    
    # 选一些关键列保存，防止 CSV 太乱，或者保存全部
    # 这里我们把评估结果放在前面方便查看
    cols = [
        'id', 
        'filter_with_meta_score', 'filter_without_meta_score', 'score_improvement',
        'filter_with_meta_reason', 'filter_without_meta_reason',
        'with_meta_data_results', 'without_meta_data_results'
    ]
    # 确保列存在 (防止某些列缺失报错)
    cols = [c for c in cols if c in output_df.columns]
    
    # 保存完整 CSV
    output_csv_path = "filtering_comparison_results.csv"
    output_df.to_csv(output_csv_path, index=False, encoding='utf-8-sig') # sig 解决中文乱码
    
    print(f"\n🎉 评估完成！结果已保存至: {output_csv_path}")
    
    # 打印简单的统计信息
    avg_with = output_df['filter_with_meta_score'].mean()
    avg_without = output_df['filter_without_meta_score'].mean()
    print(f"📊 统计摘要:\n - With Metadata Avg Score: {avg_with:.2f}\n - Without Metadata Avg Score: {avg_without:.2f}")
