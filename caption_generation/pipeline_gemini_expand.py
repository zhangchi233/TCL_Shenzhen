...
async def google_call(async_client,image,prompt):
    ...
    try:
        ...
        return result_text,token_counts
        ...
        if e.code == 503:
            ...
            return await self.google_call(async_client,image,prompt)
        else:
            raise e
...
async def process_single_caption(image,caption, context,use_openai=False,model ="gpt-4-vision-preview",api_key = "AIzaSyCjhCgDEZ05AGFkRWSGRRPCOWULbvvjOlw"):
    ...
    response,count1 = await (openai_call(client,image,step1_prompt,model) if use_openai else google_call(client,image,step1_prompt))
    entities = parse_json_list(response)
    
    step2_prompt = step2_generate_background(caption, entities)
    response,count2 = await (openai_call(client,image,step2_prompt,model) if use_openai else google_call(client,image,step2_prompt))
    
    raw_background_info = response
   
    context_response,count3 = step1_5_extract_contextual_knowledge(caption, context)
    
    step3_prompt = step3_visual_fact_check(image, caption, raw_background_info)
    response,count3 = await (openai_call(client,image,step3_prompt,model) if use_openai else google_call(client,image,step3_prompt))
    verified_background = response
    
    step4_prompt = step4_integrate_final_caption(caption, verified_background,context_response)
    response,count4 = await (openai_call(client,image,step4_prompt,model) if use_openai else google_call(client,image,step4_prompt))
