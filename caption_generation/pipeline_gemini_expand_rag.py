...
async def google_call(async_client,image,prompt):
    ...
    response = await async_client.models.generate_content(
                    ...
                    contents= [
                        await async_client.files.upload(file=image),
...
async def process_single_caption(image,caption, context,use_openai=False,model ="gpt-4-vision-preview",api_key = "AIzaSyCjhCgDEZ05AGFkRWSGRRPCOWULbvvjOlw"):
    try:
        ...
        rag_contents = run_single_test("/n".join(entities))
...
if __name__=="__main__":
    import json 
    data = "/Users/Shared/taxonomy_pipeline/temp_images/temp.json"
    data = json.load(open(data))
    images,captions,contexts = [],[],[]
    for sample in data:
        img = sample["images"][0]
        img = os.path.join("/Users/Shared/taxonomy_pipeline/temp_images",img.split("/")[-1])
    ...
    with open("/Users/Shared/taxonomy_pipeline/temp_images/expanded_captions_rag.json","w") as f:
